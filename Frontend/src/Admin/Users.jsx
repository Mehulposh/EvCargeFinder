...
const deleteData = (taskId) => {
  ...
  const fetchUserData = (userId) => {
    ...
    axios.get(`http://localhost:7000/userbookings/${userId}`)
            ...
            <td>
             <div style={{display:"flex",justifyContent:"space-around"}}>
              <button style={{ border: 'none', background: 'none' }}>
                <Link to={`/edituser/${item._id}`} style={{ color: 'blue', textDecoration: 'none' }}>
                  <FaEdit />
                </Link>
              </button>
              <button onClick={() => deleteData(item._id)} style={{ border: 'none', color: 'red', background: 'none',paddingLeft:"55px" }}>
                <FaTrash />
              </button>
              <Button onClick={() => fetchUserData(item._id)} >
                view
              </Button>
              </div>
